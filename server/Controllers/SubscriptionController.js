...
const getAllSubscriptions = async (req, res) => {
    try {
        const subscriptions = await Subscription.find().populate('categories', 'name');
...
const getSubscriptionById = async (req, res) => {
    try {
        const subscription = await Subscription.findById(req.params.id).populate('categories', 'name');
        if (!subscription) {
...
const updateSubscription = async (req, res) => {
    try {
        const subscription = await Subscription.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }).populate('categories', 'name');
        if (!subscription) {
            return res.status(404).json({ success: false, message: 'Subscription not found' });
        }
        res.status(200).json({ success: true, subscription });
      ...
      for (const user of users) {
          if (user.subscriptions && user.subscriptions.length > 0) {
              ...
              if (subscription.expirationDate && subscription.expirationDate < now) {
                  ...
                  await user.save();
                  console.log(`Subscription expired and default assigned for user: ${user._id}`);
              }
          }
      }
  } catch (error) {
