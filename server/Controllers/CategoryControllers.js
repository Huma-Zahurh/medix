...
const getCategoryById = async (req, res) => {
    try {
        const category = await Category.findOne({ slug: req.params.slug })
            ...
            .populate({
                path: 'chapters',
                select: 'name _id subject', // Include _id and subject
                populate: {
                    path: 'subject',
                    select: '_id', // Populate subject's _id
                },
            })
            .populate({
                path: 'topics',
                select: 'name slug _id chapter', // Include _id and chapter
                populate: {
                    path: 'chapter',
                    select: '_id', // Populate chapter's _id
                },
            });
